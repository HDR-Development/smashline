...
use quote::ToTokens;
use state_callback::StateCallbackAttributes;
...
#[proc_macro_error]
#[proc_macro_attribute]
pub fn event(attr: TS, input: TS) -> TS {
    let attrs = syn::parse_macro_input!(attr as StateCallbackAttributes);
    let mut function = syn::parse_macro_input!(input as syn::ItemFn);
    ...
    let installer = attrs.installer(smashline, name);
